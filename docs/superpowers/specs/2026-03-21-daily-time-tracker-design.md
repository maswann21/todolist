...
- **겹침 방지**: 같은 날짜 내 time_blocks는 시간이 겹칠 수 없음 (API에서 검증)
- **타임테이블 인터랙션**: 체크리스트에서 태스크를 먼저 선택 → 타임테이블에서 드래그로 시간 블록 색칠
- **타임테이블 범위**: 0:00~24:00 (24시간, 144칸)
- **▲ 이월 (carry)**: MVP에서는 라벨만 표시, 자동 복사 없음
